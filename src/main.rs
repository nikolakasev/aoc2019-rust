...
fn five_amplifiers_in_a_feedback_loop(intcode: &str, phase_setting: Vec<i64>) -> i64 {
    ...
    let lambda = |name: &str, tx: Sender<i64>, rx: Receiver<i64>| {
        for received in rx {
            println!("{} Got: {}", name, received);
            tx.send(received + 1);
            break;
        }
    };
    ...
    let a = thread::spawn(move || lambda("A", tx_B, rx_A));
    let b = thread::spawn(move || lambda("B", tx_C, rx_B));
    let c = thread::spawn(move || lambda("C", tx_D, rx_C));
    let d = thread::spawn(move || lambda("D", tx_E, rx_D));
...
mod tests {
    ...
    fn day7_part_two() {
        assert_eq!(
            five_amplifiers_in_a_feedback_loop(input_day7(), vec![0, 1, 2, 3, 4]),
            1
        )
    }
